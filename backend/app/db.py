...
def init_db():
    ...
    cur.execute("SELECT id, group_token FROM groups")
    rows = cur.fetchall()
    for row in rows:
        if not row["group_token"]:
            cur.execute(
                "UPDATE groups SET group_token = ?, updated_at = ? WHERE id = ?",
...
def save_group(line_group_id: str, group_name: str | None = None):
    ...
    cur.execute(
        "SELECT id, group_token FROM groups WHERE line_group_id = ?",
        (line_group_id,),
    )
...
def save_shift_submission(
    ...
    if existing:
        ...
        cur.execute("DELETE FROM shift_entries WHERE submission_id = ?", (submission_id,))
    else:
        cur.execute(
            """
...
def get_shift_board(group_id: int, target_month: str):
    ...
    if submission_map:
        placeholders = ",".join("?" for _ in submission_map.keys())
        cur.execute(
            f"""
...
def save_shift_targets(group_id: int, target_month: str, line_user_ids: list[str]):
    ...
    cur.execute(
        "DELETE FROM shift_targets WHERE group_id = ? AND target_month = ?",
        (group_id, target_month),
    )
...
def save_shift_requirements(group_id: int, target_month: str, requirements: list[dict]):
    ...
    cur.execute(
        "DELETE FROM shift_requirements WHERE group_id = ? AND target_month = ?",
        (group_id, target_month),
    )
